            ...
            <ul class="navbar-nav mr-auto">
                ...
                @if(\Illuminate\Support\Facades\Auth::check())
                    @if(\Illuminate\Support\Facades\Auth::user()->user_type != 3)
                        <li class="nav-item">
                            <a class="nav-link list-item {{ url()->current() == route('user.index.mostseen') ? 'active' : '' }}"
                               href="{{ route('user.index.mostseen', \Illuminate\Support\Facades\Auth::user()->id) }}">Phòng trọ được xem nhiều nhất <span
                ...
                <ul class="nav navbar-nav navbar-right">
                    @if(\Illuminate\Support\Facades\Auth::check())
                        <li><a class="list-item" class="btn-dangtin" href="user/dangtin"><i class="fas fa-edit"></i>
                                Đăng tin ngay</a></li>
                    @endif
                    <li class="mr-2"><a class="list-item" href="user/login"><i class="fas fa-user-circle"></i> Đăng Nhập</a>
                    </li>
